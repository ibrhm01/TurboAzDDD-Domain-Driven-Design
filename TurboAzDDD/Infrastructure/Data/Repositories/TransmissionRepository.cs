...
namespace Infrastructure.Data.Repositories
{
    public class TransmissionRepository : RepositoryBase<Transmission>, ITransmissionRepository
    {
        ...
        public override async Task<List<Transmission>> GetAllAsync()
        {
            return await _appDbContext.Set<Transmission>().Where(b => !b.IsDeleted).Include(b => b.Vehicles).ToListAsync();
        }
        public override async Task<Transmission?> GetByIdAsync(int id)
        {
            return await _appDbContext.Set<Transmission>().Where(b => !b.IsDeleted).Include(b => b.Vehicles).FirstOrDefaultAsync(x => x.Id == id);
        }
    }
}
